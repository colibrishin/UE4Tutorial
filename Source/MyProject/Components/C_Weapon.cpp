...
int32 UC_Weapon::GetRemainingAmmo() const
{
	return TotalAmmoCount - AmmoSpent;
}

int32 UC_Weapon::GetRemainingAmmoInClip() const
{
	return AmmoPerClip - AmmoSpentInClip;
}

int32 UC_Weapon::GetAmmoPerClip() const
{
	return AmmoPerClip;
}
...
void UC_Weapon::BeginPlay()
{
	...
	{
		LOG_FUNC( LogWeaponComponent , Error , "Unable to find the pick up component, weapon will not be able to pick up!" );
	}
}
...
bool UC_Weapon::ValidateAttack()
{
	...
	{
		LOG_FUNC_PRINTF(LogWeaponComponent, Log, "Cannot fire; bCanFire: %d; bCanReload: %d;", bCanFire, bCanReload);
		return false;
	}
	...
	if (!AttackObject)
	{
		LOG_FUNC_PRINTF(LogWeaponComponent, Error, "Trying to attack with no attack object %s;", *GetOwner()->GetName());
		return false;
	}
...
bool UC_Weapon::ValidateReload()
{
	...
	if (!bCanReload)
	{
		LOG_FUNC_PRINTF(LogWeaponComponent, Log, "Cannot reload; Ammo spent: %d; Total ammo: %d", AmmoSpent, TotalAmmoCount);
		return false;
	}
	...
	if (!ReloadObject)
	{
		LOG_FUNC_PRINTF(LogWeaponComponent, Error, "Attempt to reload with not reload object %s;", *GetOwner()->GetName())
		return false;
	}

	if (!GetRemainingAmmo())
	{
		LOG_FUNC_PRINTF(LogWeaponComponent, Log, "No ammo left; Ammo spent: %d; Total ammo: %d", AmmoSpent, TotalAmmoCount);
		return false;
	}
...
void UC_Weapon::HandlePickUp( IPickableObject* InPickUpObject )
{
	if ( const ACharacter* Character = Cast<ACharacter>(InPickUpObject) )
	{
		if ( const APlayerController* PlayerController = Cast<APlayerController>(Character->GetController()) )
		{
			...
			{
				AttackStartBinding = &InputComponent->BindAction( AttackAction , ETriggerEvent::Started , this , &UC_Weapon::Attack );
				AttackStopBinding = &InputComponent->BindAction( AttackAction , ETriggerEvent::Completed , this , &UC_Weapon::StopAttack );

				ReloadBinding = &InputComponent->BindAction( ReloadAction , ETriggerEvent::Started , this , &UC_Weapon::Reload );
			}
			else
			{
...
void UC_Weapon::HandleDrop( IPickableObject* InPickUpObject )
{
	if ( const ACharacter* Character = Cast<ACharacter>( InPickUpObject ) )
	{
		if ( const APlayerController* PlayerController = Cast<APlayerController>( Character->GetController() ) )
...
void UC_Weapon::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(UC_Weapon, TotalAmmoCount);
}
